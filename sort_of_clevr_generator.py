...
def build_dataset():
    ...
    for _ in range(nb_questions):
        ...
        if subtype == 0:
            ...
            if objects[color][1][0] < img_size / 2: # Check to see if the x coordinate of object's centroid falls closer to left side or right side of image
            ...
            if objects[color][1][1] < img_size / 2: # Check to see if the y coordinate of object's centroid falls closer to top side or bottom side of image
                answer = 0 # Answer: Top
            else:
    ...
    for _ in range(nb_questions):
        ...
        if subtype == 0:
            ...
            my_obj = objects[color][1] # Extract the centroid of a certain object which is chosen based on color index
            dist_list = [((my_obj - obj[1]) ** 2).sum() for obj in objects] # Store the distance between my_obj and every other object in the set in dist_list
            ...
            if objects[closest][2] == 'r': # Check to see if closest object is a rectangle
                answer = 2 
            ...
            if objects[furthest][2] == 'r': # Check to see if farthest object is a rectangle
                answer = 2
            ...
            """count->1~6"""
            my_obj = objects[color][2] # Store object to compare the rest to
            count = -1 # Set to negative one to account for the identical object in the set under the for loop
            for obj in objects:
                if obj[2] == my_obj: # Loop through each object in the image and check if shape is the same
    ...
    """Ternary Relational questions; NOTE: Code for ternary is not relevant for our project, so we will disregard but keep the code to avoid bugs"""
    for _ in range(nb_questions):
        question = jnp.zeros((question_size))
